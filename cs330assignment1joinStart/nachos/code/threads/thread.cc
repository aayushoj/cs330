...
void InitializeChild(int arg)
{
  ...
  //  machine->Debugger();
machine->Run();
//printf("testesdasdasd\n");
}


void 
NachOSThread::setuserRegisters(int *A){
  //  printf("set user registers\n");
    for (int i = 0; i < NumTotalRegs; i++) 
        userRegisters[i] = A[i];
    userRegisters[2]=0;
}
...
NachOSThread::setChild(NachOSThread* child){
   // printf("In setchild\n");
    ...
    child->space->ThreadCreateAddrSpace(currentThread->space);
    // child->space = spaceChild;

    child->ThreadFork(InitializeChild, 0);  
}
