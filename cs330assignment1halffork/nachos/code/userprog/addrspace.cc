...
TranslationEntry* getTranslationEntry(){
    return pageTable;
}

AddrSpace::ThreadCreateAddrSpace(AddrSpace* copy){

    numPages = copy->getnumPages();
    TranslationEntry* parentPageTable=copy->getTranslationEntry();
    pageTable = new TranslationEntry[numPages];
    for (i = 0; i < numPages; i++) {
    pageTable[i].virtualPage = parentPageTable[i].virtualPage;   // for now, virtual page # = phys page #
