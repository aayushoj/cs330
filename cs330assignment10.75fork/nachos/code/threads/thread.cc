...
//initializing a new global variable for unique pid
int pidcounter=0;
...
int NachOSThread::getwhentowake()
{
    return whenToWake;
}
...
void
NachOSThread::setppid(int x){
    ppid=x;
}

void
NachOSThread::setpid(int x){
    pid=x;
}

void 
NachOSThread::setuserRegisters(int *A){
...
void 
NachOSThread::setChild(NachOSThread* child){
    
    child->setppid(pid);
    child->setpid(pidcounter++);
    child->setuserRegisters(userRegisters);
    AddrSpace *spaceChild = new AddrSpace();
    spaceChild->ThreadCreateAddrSpace(currentThread->space);
    child->space = spaceChild;
    child->ThreadStackAllocate(InitializeChild, (int)0);  //not equal to 0 change it
}
